...
class EvolutionGenerator {
  String generateSummary(EvolutionData data, {String? bedLabel}) {
    ...
    section('RESPIRATÓRIO / OXIGENAÇÃO / VM', [
      ...
      if (_clean(data.inspiratoryFlow) != null)
        'Fluxo inspiratório: ${_clean(data.inspiratoryFlow)} L/min.',
      if (_clean(data.controlledPressure) != null)
        'PC: ${_clean(data.controlledPressure)} cmH2O.',
      if (_clean(data.inspiratoryTime) != null)
        'Tempo inspiratório: ${_clean(data.inspiratoryTime)} s.',
      if (_clean(data.pressureSupport) != null)
        'PS: ${_clean(data.pressureSupport)} cmH2O.',
      if (_clean(data.triggerSensitivity) != null)
        'Trigger: ${_clean(data.triggerSensitivity)}.',
      if (_clean(data.cyclingCriterion) != null)
        'Ciclagem: ${_clean(data.cyclingCriterion)}%.',
    ...
    section('HEMODINÂMICA / DVA', [
      ...
      if (data.vasoactiveDrugRates.isEmpty &&
          _clean(data.vasoactiveDrugs) != null)
        'Drogas vasoativas: ${_clean(data.vasoactiveDrugs)}.',
    ]);

    section('TEMPERATURA', [
      if (data.temperatureState != null)
        'Estado térmico: ${switch (data.temperatureState!) {
    ...
    section('DIETA / GASTROINTESTINAL / GLICEMIA', [
      ...
      if (_clean(data.gastrointestinalNotes) != null)
        'Observação gastrointestinal: ${_clean(data.gastrointestinalNotes)}.',
      if (_clean(data.hgtMinimum) != null || _clean(data.hgtMaximum) != null)
        'HGT: ${_range(data.hgtMinimum, data.hgtMaximum)} mg/dL.',
      if (_clean(data.hgtMinimum) == null &&
          _clean(data.hgtMaximum) == null &&
          _clean(data.capillaryGlucose) != null)
        'HGT: ${_clean(data.capillaryGlucose)} mg/dL.',
    ]);
    ...
    section('EVACUAÇÃO', [
      ...
      if (data.bowelMovement == BowelMovement.ausentes &&
          _clean(data.daysSinceLastBowelMovement) != null)
        'Última evacuação: há ${_clean(data.daysSinceLastBowelMovement)} dias.',
      if (data.bowelMovement != BowelMovement.ausentes &&
    ...
    section('SEDOANALGESIA / INFUSÕES', [
      ...
      if (data.sedationDrugRates.isEmpty && _clean(data.sedoanalgesia) != null)
        'Sedação/analgesia: ${_clean(data.sedoanalgesia)}.',
